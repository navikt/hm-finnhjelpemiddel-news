...
interface NewsRepository : CoroutineCrudRepository<News, UUID>, CoroutinePageableCrudRepository<News, UUID> {
    ...
    @Query(
        value = """SELECT news_.* FROM news news_ 
            ...
            AND (:active = false OR (news_.published_from <= NOW() AND news_.published_to >= NOW() AND news_.status = 'PUBLISHED'))
            AND (:status IS NULL OR news_.status = :status)""",
        countQuery = "SELECT COUNT(*) FROM news WHERE id IN (:ids) AND (:active = false OR (published_from <= NOW() AND published_to >= NOW() AND status = 'PUBLISHED')) AND (:status IS NULL OR status = :status)"
    )
    suspend fun findAllByIds(ids: List<UUID>, pageable: Pageable, active: Boolean, status: Status?): Page<News>
    ...
    @Query(
        value = """SELECT news_.* FROM news news_ 
            ...
            AND (:active = false OR (news_.published_from <= NOW() AND news_.published_to >= NOW() AND news_.status = 'PUBLISHED'))
            AND (:status IS NULL OR news_.status = :status)""",
        countQuery = "SELECT COUNT(*) FROM news WHERE (title ILIKE :search OR description ILIKE :search) AND (:active = false OR (published_from <= NOW() AND published_to >= NOW() AND status = 'PUBLISHED')) AND (:status IS NULL OR status = :status)"
    )
    suspend fun searchAll(search: String, pageable: Pageable, active: Boolean, status: Status?): Page<News>
